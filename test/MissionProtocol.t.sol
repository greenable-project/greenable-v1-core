...
contract MissionProtocolTest is Test {
    ...
    function testSubmitAttestation() public {
        ...
        // 사용자 참여 정보 확인
        MissionProtocol.UserParticipation memory participation = 
            protocol.getUserParticipation(missionId, user1);
    ...
    function testSubmitBatchAttestation() public {
        ...
        // 실제 배치 서명 생성
        bytes memory signature = _createBatchSignature(users, missionId, currentDate, dataHashes, rewards, verifier1PrivateKey);
        ...
        for (uint256 i = 0; i < users.length; i++) {
            MissionProtocol.UserParticipation memory participation = 
                protocol.getUserParticipation(missionId, users[i]);
            assertEq(participation.lastRewardDate, currentDate);
            assertEq(participation.totalDaysParticipated, 1);
